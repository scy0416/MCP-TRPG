...
엔티티, 인벤토리를 하나의 게임 스냅샷으로 반환한다. 장면의 `state.choices`가
문자열 배열 또는 `{label, description}` 객체 배열이면 Game App이 선택 버튼으로
표시하고, 사용자의 선택을 채팅에 전달한다. 결과는 읽기 중심의
`ui://trpg/game` MCP App에 표시되며, App은 서버가 반환한 값만 렌더링한다.
